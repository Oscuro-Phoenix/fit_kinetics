function [f_fit,bounds,tht_fit,cost_val,chi_squared,BIC,size_vec,xdata,ydata,xvals] = get_fit_2(temp_vec, conc_vec, filenames,rxn_type,activity)
...
fun = @(tht) cost_func_2(tht,x,y,size_vec,temp_vec,conc_vec,rxn_type,activity); %Cost function
...
if rxn_type == "ICET_asymmetric_film2" || rxn_type == "ICET_symmetric_film2"
    ...
    lb = [-Inf;-50*ones(Nukn-num_unique-2,1);-3;zeros(num_unique,1)];
    ub = [0;50*ones(Nukn-num_unique-2,1);3;10*ones(num_unique,1)];
elseif rxn_type == "ECIT_film"
    tht_i = [-100;ones(Nukn-num_unique-2,1);ones(num_unique+1,1)];
    lb = [-Inf;0;-90*ones(Nukn-num_unique-2,1);zeros(num_unique,1)];
    ub = [0;10;90*ones(Nukn-num_unique-2,1);ones(num_unique,1)];
elseif rxn_type == "BV_film"
    tht_i = rand(Nukn,1);
...
elseif rxn_type == "BV"
    ...
    tht_i = rand(Nukn,1);
    lb = [-Inf;0;-90*ones(Nukn-3,1);-1];
    ub = [0;10;90*ones(Nukn-3,1);1];
end
disp("Slope Values : \n");
disp(get_slopes(x,y,size_vec));
...
gs = GlobalSearch('NumStageOnePoints',1e1,'NumTrialPoints',50,'PenaltyThresholdFactor',0.5,...
...
[cost_val,chi_squared] = pure_cost_2(tht_fit,x,y,size_vec,temp_vec,conc_vec,rxn_type,activity);
BIC = size(xdata,1)*log(cost_val/size(xdata,1))+Nukn*log(size(xdata,1));
xvals = repmat(linspace(-10,10,200)',size(filenames,2),1);
f_x = @(tht) predict_2(xvals,tht,200*ones(size(filenames,2),1),temp_vec,conc_vec,rxn_type,activity);
[f_fit,~] = f_x(tht_fit);
Z = get_gradient(tht_fit,xdata,size_vec,temp_vec,conc_vec,rxn_type,activity);
J = get_gradient(tht_fit,xvals,200*ones(size(filenames,2),1),temp_vec,conc_vec,rxn_type,activity);
FI = pinv(Z'*Z);
R = zeros(size(xvals));
for i=1:size(xvals,1)
    R(i) = sqrt(J(i,:)*FI*J(i,:)');
end
bounds = 1.96*R*get_rmse_with_dof(tht_fit,x,y,size_vec,temp_vec,conc_vec,rxn_type,activity);

print_params_2(tht_fit,rxn_type,num_unique);
writematrix([f_fit xvals],'fitted_data.csv');
writematrix(tht_fit,'fitted_params.csv');
end